    ...
    findSlot(yardModel: YardModel, container: Container, excludeLocation?: Location): Location | null {
        ...
        for (const stack of sortedStacks) {
            ...
            if (container.type === 'REEFER') {
                if (!stack.is_reefer_compatible) continue; 
            }
            ...
            if (currentTier === 1) {
                ...
                const slotBelow = state.stacks.find(s => 
                    s.yard === stack.yard &&
                    s.block === stack.block &&
